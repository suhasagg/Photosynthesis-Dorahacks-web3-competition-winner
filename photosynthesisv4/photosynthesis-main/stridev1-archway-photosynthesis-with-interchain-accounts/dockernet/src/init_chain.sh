...
# Condition to check if chain id is 'localnet'
    ...
    log_file=$DOCKERNET_HOME/logs/${node_name}.initlog
    # Initialize the chain
    $cmd init $moniker --chain-id $CHAIN_ID --overwrite 
    echo "NodeID: IN PROGRESS1"
    chmod -R 777 $STATE/$node_name
    #echo "NodeID: IN PROGRESS2" 
...
if [ "$CHAIN" == "STRIDE" ]; then 
    ...
    echo $REV_ACCT
    echo $MAIN_CMD
    echo $REV_MNEMONIC | $MAIN_CMD keys add $REV_ACCT --recover --keyring-backend=test 
...
echo "GENTX2"
    ...
    echo "$RELAYER_MNEMONIC" | $MAIN_CMD keys add $RELAYER_ACCT --recover --keyring-backend=test 
