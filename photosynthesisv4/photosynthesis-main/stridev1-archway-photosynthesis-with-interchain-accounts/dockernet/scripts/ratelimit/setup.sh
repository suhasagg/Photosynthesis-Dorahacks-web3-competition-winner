...
setup_photo_gaia_channel() {
    print_header "CREATING PHOTO <> GAIA CHANNEL"

    relayer_exec="$DOCKER_COMPOSE run --rm relayer-photo-gaia"
    path="photo-gaia"
    ...
    mkdir -p $relayer_config
    cp ${DOCKERNET_HOME}/config/relayer_config_photo_gaia.yaml $relayer_config/config.yaml

    printf "JUNO <> OSMO - Adding relayer keys..."
    RELAYER_PHOTO_MNEMONIC="toddler they unveil hard produce laundry box chicken pink ocean space autumn regret immune sort fame hat common beauty butter candy miracle brave fun"
    RELAYER_GAIA_MNEMONIC="solution simple collect warrior neither grain ethics dust guard high base hamster sail science valley organ mistake soon letter garden october morning correct hidden"
    PHOTO_RELAYER_ADDRESS=$($relayer_exec rly keys restore juno juno-osmo-rly1 "$RELAYER_PHOTO_MNEMONIC") 
    GAIA_RELAYER_ADDRESS=$($relayer_exec rly keys restore osmo juno-osmo-rly2 "$RELAYER_GAIA_MNEMONIC")
    echo "Done"

    printf "PHOTO <> GAIA - Funding Relayers...\n" 
    $PHOTO_MAIN_CMD tx bank send ${PHOTO_VAL_PREFIX}1 $PHOTO_RELAYER_ADDRESS 1000000uatom --from ${PHOTO_VAL_PREFIX}1 -y | TRIM_TX
    $GAIA_MAIN_CMD tx bank send ${GAIA_VAL_PREFIX}1 $GAIA_RELAYER_ADDRESS 1000000uatom --from ${GAIA_VAL_PREFIX}1 -y | TRIM_TX
    sleep 3
    echo "Done"

    printf "PHOTO <> GAIA - Creating client, connection, and transfer channel..." | tee -a $relayer_logs
    $relayer_exec rly transact link $path >> $relayer_logs 2>&1
    echo "Done"
...
setup_channel_value() {
    ...
    echo "Transfering for channel value..."
    echo ">>> uatom"
    $PHOTO_MAIN_CMD tx ibc-transfer transfer transfer channel-0 $(STRIDE_ADDRESS) ${INITIAL_CHANNEL_VALUE}uatom --from ${GAIA_VAL_PREFIX}1 -y | TRIM_TX
    sleep 3
    ...
    echo ">>> traveler-uatom"
    photo_on_gaia='ibc/448C1061CE97D86CC5E86374CD914870FB8EBA16C58661B5F1D3F46729A2422D'
    $PHOTO_MAIN_CMD tx ibc-transfer transfer transfer channel-5 $(GAIA_ADDRESS) ${INITIAL_CHANNEL_VALUE}ujuno --from ${PHOTO_VAL_PREFIX}1 -y | TRIM_TX
    sleep 10
    $GAIA_MAIN_CMD tx ibc-transfer transfer transfer channel-0 $(STRIDE_ADDRESS) ${INITIAL_CHANNEL_VALUE}${photo_on_gaia} --from ${GAIA_VAL_PREFIX}1 -y | TRIM_TX
    sleep 3
