...
Learn how to generate, sign and broadcast a transaction using the keyring
multisig {synopsis}

A **multisig account** is an Evmos account with a special key that can require
more than one signature to sign transactions. This can be useful for increasing
...
combined into a multisignature which can be used to sign the transaction. If
fewer than the threshold number of signatures needed are present, the resultant
...
Unless the flag `--nosort` is set, the order in which the keys are supplied on
the command line does not matter, i.e. the following commands generate two
identical keys:
...
Let's assume that you have `test1` and `test2` want to make a multisig account
with `test3`.
...
We want to send 5 EVMOS from our multisig account to
