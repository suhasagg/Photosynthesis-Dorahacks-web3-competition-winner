...
fn stake_tokens(
    ...
    let amount = info.funds.iter().find(|c| c.denom == "ujwc").map(|c| c.amount).unwrap_or(Uint128::zero());

    if amount.is_zero() {
        println!("No ujwc tokens sent for staking.");
        return Err(StdError::generic_err("Must send non-zero amount of ujwc tokens"));
    }
    ...
    println!("Staking {} ujwc tokens for staker: {}", amount, staker);
...
fn unstake_tokens(
    ...
    println!("Unstaking {} ujwc tokens for staker: {}", current_stake, staker);
    STAKES.save(deps.storage, &staker, &Uint128::zero())?;

    Ok(Response::new()
        .add_message(BankMsg::Send {
            to_address: staker.to_string(),
            amount: vec![coin(current_stake.u128(), "ujwc")],
        })
...
mod tests {
    ...
    fn contract_john_wick() -> Box<dyn Contract<Empty>> {
...
fn test_stake_tokens() {
    ...
    let code_id = app.store_code(contract_john_wick());
