...
const CONTRACT_NAME: &str = "crates.io:wwe_coin";
...
fn stake_tokens(
    ...
    let amount = info.funds.iter().find(|c| c.denom == "uwwec").map(|c| c.amount).unwrap_or(Uint128::zero());

    if amount.is_zero() {
        println!("No uwwec tokens sent for staking.");
        return Err(StdError::generic_err("Must send non-zero amount of uwwec tokens"));
    }
    ...
    println!("Staking {} uwwec tokens for staker: {}", amount, staker);
...
fn unstake_tokens(
    ...
    println!("Unstaking {} uwwec tokens for staker: {}", current_stake, staker);
    STAKES.save(deps.storage, &staker, &Uint128::zero())?;

    Ok(Response::new()
        .add_message(BankMsg::Send {
            to_address: staker.to_string(),
            amount: vec![coin(current_stake.u128(), "uwwec")],
        })
