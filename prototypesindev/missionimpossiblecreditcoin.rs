use cosmwasm_std::{attr, coin, to_binary, BankMsg, Binary, Deps, DepsMut, Env, HandleResponse, InitResponse, MessageInfo, StdError, StdResult};
...
fn stake_tokens(
    ...
    let amount = info.sent_funds.iter().find(|c| c.denom == "umic").map(|c| c.amount.u128()).unwrap_or(0);
