...
const CONTRACT_NAME: &str = "crates.io:matrix_token";
...
fn stake_tokens(
    ...
    let amount = info.funds.iter().find(|c| c.denom == "umtx").map(|c| c.amount).unwrap_or(Uint128::zero());

    if amount.is_zero() {
        println!("No umtx tokens sent for staking.");
        return Err(StdError::generic_err("Must send non-zero amount of umtx tokens"));
    }
    ...
    println!("Staking {} umtx tokens for staker: {}", amount, staker);
...
fn unstake_tokens(
    ...
    println!("Unstaking {} umtx tokens for staker: {}", current_stake, staker);
    STAKES.save(deps.storage, &staker, &Uint128::zero())?;

    Ok(Response::new()
        .add_message(BankMsg::Send {
            to_address: staker.to_string(),
            amount: vec![coin(current_stake.u128(), "umtx")],
        })
...
mod tests {
    ...
    fn contract_matrix() -> Box<dyn Contract<Empty>> {
...
fn test_stake_tokens() {
    ...
    let code_id = app.store_code(contract_matrix());
