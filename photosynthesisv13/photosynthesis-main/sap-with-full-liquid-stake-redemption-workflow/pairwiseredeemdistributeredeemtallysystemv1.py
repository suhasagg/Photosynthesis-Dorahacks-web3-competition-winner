...
logging.basicConfig(level=logging.DEBUG, 
                    ...
                    filename='/media/usbHDD1/redeemtokensdistributeredeemamountstally.log', 
                    filemode='a') 
...
def extract_distribute_redeem_amount(message):
...
def timestamp_tally(logs1):
    ...
    Returns:
    - List[Tuple[int, int]]: Pairs of accumulated amounts for central redeem stuarch (at maximum redemption rate) and claimed amount
    """
    ...
    for log_entry in logs1:
        ...
        # Extracting amounts
        claimed_amount = extract_amount(str(log_entry))
    ...
    logger.debug(f"Processed {len(claimed_data)} reward logs with non-zero amounts.")
    amount = 0  
