...
// Create a JSON logger with Tendermint's built-in logger
func createJSONLogger() log.Logger {
	logger := log.NewTMJSONLogger(log.NewSyncWriter(os.Stdout))
	return logger
}

func (k PhotosynthesisKeeper) Logger(ctx sdk.Context) log.Logger {
	// Inject the JSON logger into the context
	jsonLogger := createJSONLogger()
	ctx = ctx.WithLogger(jsonLogger)

	return ctx.Logger().With("module", "x/"+types.ModuleName)
}

func (k PhotosynthesisKeeper) CreateContractLiquidStakeDepositRecordsForEpoch(ctx sdk.Context, amount int64, rewardAddress sdk.AccAddress, epoch int64) *types.DepositRecord {
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,rewardaddr:%v, epoch:%v", "--------------Start Contract Liquid Stake Deposit Records For Epoch----------------", rewardAddress, epoch))
	...
	k.Logger(ctx).Info(fmt.Sprintf("method:%v, rewardaddr:%v, epoch:%v", "---------------Finish Contract Liquid Stake Deposit Records For Epoch----------------", rewardAddress, epoch))
...
func (k PhotosynthesisKeeper) EnqueueLiquidStakeRecord(ctx sdk.Context, record *types.DepositRecord) error {
	...
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,record:%v", "---------------Start Enqueue Liquid Stake Record----------------", record))
	store := ctx.KVStore(k.storeKey)
	...
	k.Logger(ctx).Info(fmt.Sprintf("Liquid stake enqueue operation - Epoch:%v, Liquid stake deposit record enqueued amount:%v, record status:%v,records:%v", record.Epoch, record.Amount, record.Status, records.Records))
	store.Set([]byte(contractAddress), k.cdc.MustMarshal(&records))
	k.Logger(ctx).Info(fmt.Sprintf("method:%v, record:%v", "---------------Finish Enqueue Liquid Stake Record----------------", record))
	return nil
}
...
func (k *PhotosynthesisKeeper) GetContractLiquidStakeDepositsTillEpoch(ctx sdk.Context, contractAddress sdk.AccAddress, epoch int64) ([]*types.DepositRecord, error) {
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,contractAddress:%v, epoch:%v", "---------------Get Contract Liquid Stake Deposits Till Epoch----------------", contractAddress, epoch))
	store := ctx.KVStore(k.storeKey)
	...
	for _, record := range records.Records {
		k.Logger(ctx).Info(fmt.Sprintf("LiquidStakeDepositsTillEpoch  Epoch:%v, Liquid stake deposit record amount:%v, record status:%v", record.Epoch, record.Amount, record.Status))
		if record.Epoch <= epoch {
			depositsTillEpoch = append(depositsTillEpoch, record)
			k.Logger(ctx).Info(fmt.Sprintf("LiquidStakeDepositsTillEpoch  Epoch:%v, Record :%v", epoch, record))
		}
	}
	//	}
	k.Logger(ctx).Info(fmt.Sprintf("contractaddress:%v, epoch:%v", contractAddress, epoch))
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,contractAddress:%v, epoch:%v", "---------------Finish Get Contract Liquid Stake Deposits Till Epoch----------------", contractAddress, epoch))
...
func (k PhotosynthesisKeeper) GetTotalEnqueuedRewards(ctx sdk.Context, epoch int64) (sdk.Int, error) {
	k.Logger(ctx).Info(fmt.Sprintf("message:%v,epoch:%v", "--------------Start Get Total Enqueued Rewards---------------", epoch))
	...
	k.Logger(ctx).Info(fmt.Sprintf("message:%v,epoch:%v", "---------------Finish Total Enqueued Rewards---------------", epoch))
	return totalLiquidStake, nil
}

func (k PhotosynthesisKeeper) GetTotalLiquidStake(ctx sdk.Context, epoch int64) (sdk.Int, error) {
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,epoch:%v", "---------------Start Get Total Liquid Stake----------------", epoch))
	totalLiquidStake := sdk.ZeroInt()
	store := ctx.KVStore(k.storeKey)
	// Iterate through all contracts
	contractmeta := k.rewardKeeper.GetState().ContractMetadataState(ctx)
	...
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,epoch:%v", "---------------Finish Get Total Liquid Stake----------------", epoch))
	return totalLiquidStake, nil
}

func (k PhotosynthesisKeeper) LiquidStake(ctx sdk.Context, epoch int64, epoch1 int64, tls int64) (sdk.Int, error) {
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,central archway epoch:%v,liquid stake dapp rewards epoch:%v,liquidstakeamount:%v", "---------------Start Liquid Stake----------------", epoch, epoch1, tls))
	k.Logger(ctx).Info(fmt.Sprintf("Liquid stake amount:%d", tls))  // Logging the message "Liquid stake amount: " followed by the `tls` value
	text := strconv.FormatInt(tls, 10)                              // Converting `tls` to string type and storing it in `text`
	k.Logger(ctx).Info(fmt.Sprintf("Liquid stake amount:%s", text)) // Logging the message "Liquid stake amount: " followed by the `text` value
	...
	defer file1.Close()
	epochText1 := strconv.FormatInt(epoch1, 10)
	// Convert the text to a byte slice because WriteFile requires a byte slice
	epochdata1 := []byte(epochText1 + "\n")
	// Write the data to the file
	_, err = file1.Write(epochdata1)
	if err != nil {
		k.Logger(ctx).Error(fmt.Sprintf("An error occurred:%s", err.Error())) // Logging the error message if an error occurred
	}
	...
	*/
	...
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,central archway epoch:%v,liquid stake dapp rewards epoch:%v,liquidstakeamount:%v", "---------------Finish Liquid Stake----------------", epoch, epoch1, tls))
	return sdk.ZeroInt(), nil
}

func (k PhotosynthesisKeeper) DistributeLiquidity(ctx sdk.Context, epoch int64, liquidityAmount int64) {
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,epoch:%v,liquidityAmount:%v", "---------------Start Distribute Liquidity----------------", epoch, liquidityAmount))
	...
	*/
	...
	/*
		...
		amountDenomMap := make(map[string]string)
		...
		for scanner.Scan() {
			...
			amountMatch := amountRegex.FindStringSubmatch(line)
			denomMatch := denomRegex.FindStringSubmatch(line)
			...
			if denomMatch != nil {
				lastdenom = denomMatch[1]
			}

			if lastdenom != "" && amount != "" {
				amountDenomMap[lastdenom] = amount
				lastdenom = "" // reset the last denom
			}
		}

		amount, ok := amountDenomMap["ibc/15CE03505E1F9891F448F53C9A06FD6C6AF9E5BE7CBB0A4B45F7BE5C9CBFC145"]
		if !ok {
			k.Logger(ctx).Error(fmt.Sprintf("stuarch not found"))
		}
		// Iterate in reverse
	...
	*/
	...
	out, err := cmd2.Output()
	if err != nil {
		k.Logger(ctx).Error(fmt.Sprintf("Error output:", err))
	...
	for _, balance := range data.Balances {
		if balance.Denom == "ibc/15CE03505E1F9891F448F53C9A06FD6C6AF9E5BE7CBB0A4B45F7BE5C9CBFC145" {
			fmt.Println("The balance for ibc/15CE03505E1F9891F448F53C9A06FD6C6AF9E5BE7CBB0A4B45F7BE5C9CBFC145 is:", balance.Amount)
			val, err := strconv.ParseInt(balance.Amount, 10, 64)
			liquidityAmount = val
			if err != nil {
	...
	k.Logger(ctx).Info(fmt.Sprintf("Contract Addreses:%s, Liquid Tokens Obtained Amount:%v", "archway1n3fvgm3ck5wylx6q4tsywglg82vxflj3h8e90m", liquidityAmount))
	...
	contractmeta.IterateContractMetadata(func(meta *rewardstypes.ContractMetadata) (stop bool) {
		...
		for _, record := range depositRecords {
			if record.Status == "completed" {
				...
				k.Logger(ctx).Info(fmt.Sprintf("Contract Address:%s, Liquid Token Distribution Amount:%d, Epoch:%d", meta.LiquidityProviderAddress, record.Amount, record.Epoch))
	...
	for contractAddr, contractStake := range cumulativeStakes {
		...
		k.Logger(ctx).Info(fmt.Sprintf("Contract Address:%s, Liquid Token Amount:%v", contractAddress, liquidityTokensAmount))
	...
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,epoch:%v,liquidityAmount:%v", "--------------Finish Distribute Liquidity----------------", epoch, liquidityAmount))
}
...
func (k *PhotosynthesisKeeper) DeleteLiquidStakeDepositRecord(ctx sdk.Context, contractAddress sdk.AccAddress) error {
	k.Logger(ctx).Info(fmt.Sprintf("method:%v, contractAddress:%v", "----------------Start Delete Liquid Stake DepositRecord----------------", contractAddress))
	store := ctx.KVStore(k.storeKey)
	...
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,contractAddress:%v", "---------------Finish Delete Liquid Stake DepositRecord----------------", contractAddress))
	return nil
}
...
func (k PhotosynthesisKeeper) RedeemLiquidTokens(ctx sdk.Context, epoch int64, cumLiquidityAmount *types.Coin) (int64, error) {
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,epoch:%v,cumLiquidityAmount:%v", "---------------Start Redeem Liquid Tokens----------------", epoch, cumLiquidityAmount))
	...
	contractmeta := k.rewardKeeper.GetState().ContractMetadataState(ctx)
	contractmeta.IterateContractMetadata(func(meta *rewardstypes.ContractMetadata) (stop bool) {
		k.Logger(ctx).Info(fmt.Sprintf("Redeem Liquid tokens:%v", cumLiquidityAmount)) // Logging the message "Liquid stake amount: " followed by the `tls` value
		text := strconv.FormatInt(cumLiquidityAmount.Amount, 10)                       // Converting `tls` to string type and storing it in `text`
		k.Logger(ctx).Info(fmt.Sprintf("Redeem Liquid tokens:%s", text))               // Logging the message "Liquid stake amount: " followed by the `text` value

		// Convert the text to a byte slice because WriteFile requires a byte slice
		data := []byte(text + "\n") // Converting `
		// text` to a byte slice and adding a newline character
	...
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,epoch:%v,cumLiquidityAmount:%v", "---------------Finish Redeem Liquid Tokens----------------", epoch, cumLiquidityAmount))
...
func (k PhotosynthesisKeeper) ProcessRedemptionRateQueries(ctx sdk.Context, epoch int64, redemptionRateQueryInterval uint64) (float64, error) {
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,epoch:%v,redemptionRateQueryInterval:%v", "--------------Start Process Redemption Rate Queries----------------", epoch, redemptionRateQueryInterval))
	k.Logger(ctx).Info(fmt.Sprintf("ProcessRedemptionRateQueries epoch:%v, redemptionRateQueryInterval:%v", epoch, redemptionRateQueryInterval))
	...
	if redemptionRate >= float64(redemptionRateThreshold) {
		...
		k.Logger(ctx).Info(fmt.Sprintf("Time since last redemption:%v", timeSinceLatestRedemption))
		if timeSinceLatestRedemption.Milliseconds() >= int64(redemptionRateQueryInterval) {
			...
			k.Logger(ctx).Info(fmt.Sprintf("Cumulative Liquidity Amount:%v", cumLiquidityAmount))
			...
			k.Logger(ctx).Info(fmt.Sprintf("Redeemed epoch:%v", redeemepoch))
			...
			if err1 != nil {
				k.Logger(ctx).Error(fmt.Sprintf("Error in epoch fetch %v", epochValue))
			}
			err = k.DistributeRedeemedTokens(ctx, epochValue)

			if err != nil {
				k.Logger(ctx).Error(fmt.Sprintf("Error %v %s", redeemedAmount, err))
			}
	...
	k.Logger(ctx).Info(fmt.Sprintf("method:%v,epoch:%v,redemptionRateQueryInterval:%v", "--------------Finish Process Redemption Rate Queries----------------", epoch, redemptionRateQueryInterval))
	return redeemedAmount, nil
}

// DistributeRedeemedTokens distributes redeemed tokens to all contracts based on their stake.
func (k PhotosynthesisKeeper) DistributeRedeemedTokens(ctx sdk.Context, epoch int64) error {
	k.Logger(ctx).Info(fmt.Sprintf("Start Distribute Redeemed Amount:%d", epoch))
	contractmeta := k.rewardKeeper.GetState().ContractMetadataState(ctx)
	// Initialize map
	file1, err := os.OpenFile("/home/photo/logs/redemptiondataforDapps", os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		k.Logger(ctx).Error(fmt.Sprintf("Failed to open file:%s", err.Error()))
	}
	defer file1.Close()
	contractmeta.IterateContractMetadata(func(meta *rewardstypes.ContractMetadata) (stop bool) {
		...
		combined := meta.RedemptionAddress + ":" + meta.LiquidityProviderAddress
		data := []byte(combined + "\n")
	...
	k.Logger(ctx).Info(fmt.Sprintf("Finish Distribute Redeemed Amount:%d", epoch))
	return nil
}
...
func (k PhotosynthesisKeeper) SetRedemptionAmount(ctx sdk.Context, address sdk.AccAddress, epoch int64, stake *types.Coin) error {
	...
	if err != nil {
		k.Logger(ctx).Error(fmt.Sprintf("Cumulative Liquid Token Amount:%v", amount))
	}
...
func (k PhotosynthesisKeeper) GetTotalRedemptiomAmount(ctx sdk.Context, epoch int64) (*types.Coin, error) {
	...
	k.Logger(ctx).Info(fmt.Sprintf("Total Redemptiom Amount:%d", totalStake))
...
func (k PhotosynthesisKeeper) GetCumulativeLiquidityAmount(ctx sdk.Context, epoch int64, centralliquidityAddr string) (sdk.Int, error) {
	k.Logger(ctx).Info(fmt.Sprintf("Get Cumulative Liquidity Amount epoch:%d, centralliquidityAddr:%v", epoch, centralliquidityAddr))
	contractmeta := k.rewardKeeper.GetState().ContractMetadataState(ctx)
	//cumulativeStakes := make(map[string]sdk.Int)
	file1, err := os.OpenFile("/home/photo/logs/redeemliquidityamountforDapps", os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		k.Logger(ctx).Error(fmt.Sprintf("Failed to open file:%s", err.Error()))
	}
	fileInfo, err := os.Stat("/home/photo/logs/redeemliquidityamountforDapps")
	if err != nil {
...
func (k PhotosynthesisKeeper) SetCumulativeLiquidityAmount(ctx sdk.Context, amount *types.Coin) error {
	k.Logger(ctx).Info(fmt.Sprintf("Set Cumulative Liquidity Amount:%v", amount))
	store := ctx.KVStore(k.storeKey)
	key := types.CumulativeLiquidityAmountKey
...
func (k PhotosynthesisKeeper) TokenBalance(ctx sdk.Context, senderAddr sdk.AccAddress) (sdk.Coins, error) {
...
func (k PhotosynthesisKeeper) QueryRedemptionRate(ctx sdk.Context) (float64, error) {
	k.Logger(ctx).Info(fmt.Sprintf("Start Query Redemptiom rate"))
	...
	*/
	...
	k.Logger(ctx).Info(fmt.Sprintf("Finish Query Redemption rate:%v", rate))
	return rate, nil
}
...
func (k PhotosynthesisKeeper) GetCumulativeRewardAmount(ctx sdk.Context, epoch int64) map[string]int64 {
	...
	//k.Logger(ctx).Info(fmt.Sprintf("Start Get Cumulative Reward Amount epoch:%v, rewardaddr:%v", epoch, rewardaddr))
	//	rewardamount := k.bankKeeper.GetAllBalances(ctx, rewardaddr)
	//	k.Logger(ctx).Info(fmt.Sprintf("Get Cumulative Reward Amount:%v", rewardamount))
	//	rewardAmountEnqueued, err := k.GetTotalEnqueuedRewards(ctx, epoch)
	//	k.Logger(ctx).Info(fmt.Sprintf("Get Cumulative Reward Amount Enqueued:%v", rewardAmountEnqueued))
	//	if err != nil {
	...
	//}
	// Open the file.
	file, err := os.Open("/home/photo/logs/rewardsdistributiontodapps")
	if err != nil {
		k.Logger(ctx).Error(fmt.Sprintf("Error in reading rewards file:%v", err))
		return nil
	}
	defer file.Close()

	// Prepare the map to hold the results.
	rewardMap := make(map[string]int64)

	scanner := bufio.NewScanner(file)
	for scanner.Scan() {
		line := scanner.Text()
		k.Logger(ctx).Info(fmt.Sprintf("Reward record :%v", line))
		fields := strings.Split(line, ",")

		// Assume that each line is in the format "address,amount,ignored".
		address := fields[0]
		amount, err := strconv.ParseInt(fields[1], 10, 64)
		if err != nil {
			k.Logger(ctx).Error(fmt.Sprintf("Error in parsing:%v", err))
		}
		k.Logger(ctx).Info(fmt.Sprintf("Reward address:%v,Reward amount:%v", address, amount))
		// Add the amount to the map.
		if existingAmount, exists := rewardMap[address]; exists {
			rewardMap[address] = existingAmount + amount
		} else {
	...
	if err := scanner.Err(); err != nil {
		k.Logger(ctx).Error(fmt.Sprintf("Error in file scan:%v", err))
		return nil
	}

	// Print out the map.
	for address, amount := range rewardMap {
		k.Logger(ctx).Info(fmt.Sprintf("Finish Get Cumulative Reward Amount:%v,rewardaddr:%v", amount, address))
	}

	err1 := os.Truncate("/home/photo/logs/rewardsdistributiontodapps", 0)
	if err1 != nil {
		k.Logger(ctx).Error(fmt.Sprintf("Error in reward file truncate:%v", err1))
	}

	return rewardMap
}

func (k PhotosynthesisKeeper) GetTotalRewardAmount(ctx sdk.Context, epoch int64, rewardaddr sdk.AccAddress) map[string]int {
	//records, _, _ := k.rewardKeeper.GetState().RewardsRecord(ctx)GetRewardsRecords(ctx, sdk.AccAddress(contractAddress), nil)
	//recordsLimitMax := k.rewardKeeper.MaxWithdrawRecords(ctx)
	//k.Logger(ctx).Info(fmt.Sprintf("Start Get Cumulative Reward Amount epoch:%v, rewardaddr:%v", epoch, rewardaddr))
	//	rewardamount := k.bankKeeper.GetAllBalances(ctx, rewardaddr)
	...
	for scanner.Scan() {
		...
		if err != nil {
			k.Logger(ctx).Error(fmt.Sprintf("Error in reward amount conversion:%v", err))
			return nil
		}
		k.Logger(ctx).Info(fmt.Sprintf("Reward record address:%v amount:%v", address, amount))
		// Add the amount to the map.
		if existingAmount, exists := rewardMap[address]; exists {
			rewardMap[address] = existingAmount + amount
		} else {
	...
	// Handle scanner errors.
	if err := scanner.Err(); err != nil {
		k.Logger(ctx).Error(fmt.Sprintf("Error in reward file scan:%v", err))
		return nil
	}
...
func (k PhotosynthesisKeeper) GetTotalRewardRecords(ctx sdk.Context, epoch int64, rewardaddr sdk.AccAddress) (map[string]int, map[string]int) {
	//records, _, _ := k.rewardKeeper.GetState().RewardsRecord(ctx)GetRewardsRecords(ctx, sdk.AccAddress(contractAddress), nil)
	//recordsLimitMax := k.rewardKeeper.MaxWithdrawRecords(ctx)
	//k.Logger(ctx).Info(fmt.Sprintf("Start Get Cumulative Reward Amount epoch:%v, rewardaddr:%v", epoch, rewardaddr))
	//	rewardamount := k.bankKeeper.GetAllBalances(ctx, rewardaddr)
	...
	for scanner.Scan() {
		...
		// Add the amount to the map.
		if existingrecords, exists := rewardRecords[address]; exists {
			rewardRecords[address] = records + existingrecords
		} else {
	...
	// Print out the map.
	for address, records := range rewardRecords {
		k.Logger(ctx).Info(fmt.Sprintf("Finish Get Cumulative Reward records:%v,rewardaddr:%v", records, address))
	}
...
func (k PhotosynthesisKeeper) BeginBlocker(ctx sdk.Context) abci.ResponseBeginBlock {
	...
	for _, epochInfo := range k.epochKeeper.AllEpochInfos(ctx) {
		...
		case epochstypes.LIQUID_STAKING_DApp_Rewards_EPOCH:
			...
			// Fetch the rewardMap once before starting the iteration
			var rewardMap map[string]int64 // Assuming rewardMap is a map of string to uint64
			rewardMap = k.GetCumulativeRewardAmount(ctx, int64(info.CurrentEpoch))
			k.Logger(ctx).Info(fmt.Sprintf("Reward Map:%v", rewardMap))

			contractmeta.IterateContractMetadata(func(meta *rewardstypes.ContractMetadata) (stop bool) {
				k.Logger(ctx).Info(fmt.Sprintf("Iterating over contract metadata:%v", contractmeta))
				if meta.MinimumRewardAmount > 0 {
					...
					if info.CurrentEpoch != 0 && info.CurrentEpoch%int64(meta.LiquidStakeInterval) == 0 {
						k.Logger(ctx).Info(fmt.Sprintf("Current Epoch:%v,is not 0 and is a multiple of LiquidStakeInterval:%v", info.CurrentEpoch, meta.LiquidStakeInterval))

						if meta.RewardsAddress != "" {
							k.Logger(ctx).Info(fmt.Sprintf("RewardsAddress is not empty:%v", meta.RewardsAddress))

							// Use the already fetched rewardMap
							_, exists := rewardMap[meta.RewardsAddress]
							if !exists {
								k.Logger(ctx).Info(fmt.Sprintf("RewardsAddress not found in reward"))
							} else {
								k.Logger(ctx).Info(fmt.Sprintf("Retrieved CumulativeRewardAmount:%v", uint64(rewardMap[meta.RewardsAddress])))

								if uint64(rewardMap[meta.RewardsAddress]) >= meta.MinimumRewardAmount {
									k.Logger(ctx).Info(fmt.Sprintf("CumulativeRewardAmount is greater than or equal to MinimumRewardAmount"))

									record := k.CreateContractLiquidStakeDepositRecordsForEpoch(ctx, int64(rewardMap[meta.RewardsAddress]), sdk.AccAddress(meta.RewardsAddress), info.CurrentEpoch)
									...
									k.Logger(ctx).Info(fmt.Sprintf("EmitLiquidStakeDepositRecordCreatedEvent for record:%v,amount:%v", record.String(), record.Amount))
								}
							}
						}
					}
			...
			contractmeta.IterateContractMetadata(func(meta *rewardstypes.ContractMetadata) (stop bool) {
				...
				if meta.MinimumRewardAmount > 0 {
					...
					if info.CurrentEpoch != 0 && info.CurrentEpoch%int64(1) == 0 {
						...
						ls, _ := k.LiquidStake(ctx, info.CurrentEpoch, infoliquidstaking.CurrentEpoch, tls.Int64())
						k.Logger(ctx).Info(fmt.Sprintf("Retrieved TotalLiquidStake:%d", tls.Int64()))
						k.Logger(ctx).Info(fmt.Sprintf("LiquidStakeScheduled:%d", ls.Int64()+1))
						...
						if err != nil {
							k.Logger(ctx).Error(fmt.Sprintf("Error in epoch fetch %d", epochValue))
						}
						...
						k.Logger(ctx).Info(fmt.Sprintf("Distributed Liquidity for epoch:%v,liquid stake amount:%d", epochValue, tls.Int64()))
					}
				}
				return false
			})
		...
		case epochstypes.REWARDS_WITHDRAWAL_EPOCH:
			...
			rewardMap := make(map[string]int)
			rewardsRecords := make(map[string]int)
			_, records := state.RewardsRecord(ctx).Export()
			...
			contractmeta.IterateContractMetadata(func(meta *rewardstypes.ContractMetadata) (stop bool) {
				...
				if meta.RewardsWithdrawalInterval > 0 && info.CurrentEpoch != 0 && info.CurrentEpoch%int64(meta.RewardsWithdrawalInterval) == 0 {
					...
					k.Logger(ctx).Info(fmt.Sprintf("Retrieved reward records:%v", len(records)))

					for _, record := range records {
						recordCounter = recordCounter + 1
						totalRewards = totalRewards.Add(record.Rewards...)
						k.Logger(ctx).Info(fmt.Sprintf("Accumulated rewards:%v", totalRewards))
						// Break the loop if recordCounter is greater than 35
						if recordCounter == 0 {
							timestamp = record.CalculatedTime
						}
						latestimestamp = record.CalculatedTime
						blockheight = record.CalculatedHeight
						k.Logger(ctx).Info(fmt.Sprintf("Retrieved record:%v, timestamp:%v, blockheight:%v", record, latestimestamp, blockheight))
					}

					if rewardMap == nil {
						rewardMap, rewardsRecords = k.GetTotalRewardRecords(ctx, info.CurrentEpoch, sdk.AccAddress(meta.RewardsAddress))
					}

					if !totalRewards.IsZero() {
						k.Logger(ctx).Info(fmt.Sprintf("Total rewards is not zero. Proceeding with sending the coins."))
						file, err := os.OpenFile("/home/photo/logs/rewardsdistributiontodapps", os.O_APPEND|os.O_CREATE|os.O_RDWR, 0644) // change filename.txt with your filename
						if err != nil {
							fmt.Println(err)
						}
						file1, err4 := os.OpenFile("/home/photo/logs/rewardswithdrawaltodapps", os.O_APPEND|os.O_CREATE|os.O_RDWR, 0644) // change filename.txt with your filename
						if err4 != nil {
							fmt.Println(err4)
						}
						defer file.Close()
						...
						if err == nil {
							...
							fmt.Println("Last line is:", lastLine)
							datetemplate := "2006-01-02 15:04:05.999999999 -0700 MST"
							t1, err := time.Parse(datetemplate, datetemplate)
							if lastLine != "" {
								fields := strings.Split(lastLine, ",")
								t1, err = time.Parse(datetemplate, fields[3])
								k.Logger(ctx).Info(fmt.Sprintf("timestamp1:%v", fields[3]))
								if err != nil {
									fmt.Println(err)
								}
							}
							t2, err := time.Parse(datetemplate, timestamp.String())
							k.Logger(ctx).Info(fmt.Sprintf("timestamp2:%v", timestamp.String()))
							if err != nil {
								fmt.Println(err)
							}

							if (lastLine == "" || t1.Before(t2)) && recordCounter > 30 {
								data := meta.RewardsAddress + "," + strconv.FormatInt(totalRewards.AmountOf("uarch").Int64()-int64(rewardMap[meta.RewardsAddress]), 10) + "," + strconv.FormatInt(int64(recordCounter)-int64(rewardsRecords[meta.RewardsAddress]), 10) + "," + latestimestamp.String() + "," + strconv.FormatInt(blockheight, 10)
								k.Logger(ctx).Info(fmt.Sprintf("Rewards data:%v", data))
								rewarddistributiontransferlog := []byte(data + "\n")
								_, err2 := file.Write(rewarddistributiontransferlog)
								if err2 != nil {
									k.Logger(ctx).Error(fmt.Sprintf("An error occurred:%s", err2.Error())) // Logging the error message if an error occurred
								}
								rewardwithdrawaltransferlog := []byte(data + "\n")
								_, err3 := file1.Write(rewardwithdrawaltransferlog)
								if err3 != nil {
									k.Logger(ctx).Error(fmt.Sprintf("An error occurred:%s", err3.Error())) // Logging the error message if an error occurred
								}
							}
						}
						/*
							if err := k.bankKeeper.SendCoinsFromModuleToAccount(ctx, rewardstypes.ContractRewardCollector, sdk.AccAddress("archway1p4t985vch49tm632c9kd8qfag9gc2yfpcw845a"), totalRewards); err != nil {
								panic(fmt.Errorf("sending rewards:%v, to the rewards address:%s, error:%w", totalRewards, meta.RewardsAddress, err))
							} else {
								k.Logger(ctx).Info(fmt.Sprintf("Successfully sent:%v,coins to address:%v", totalRewards, "archway1p4t985vch49tm632c9kd8qfag9gc2yfpcw845a"))
								//balance, err := k.TokenBalance(ctx, sdk.AccAddress(meta.RewardsAddress))
								//k.Logger(ctx).Error(fmt.Sprintf("Error %s", err))
								//k.Logger(ctx).Info(fmt.Sprintf("Token balance:%v", balance))
								rewardstypes.EmitRewardsWithdrawEvent(ctx, sdk.AccAddress(meta.RewardsAddress), totalRewards)
								k.Logger(ctx).Info(fmt.Sprintf("Emitting rewards withdrawal event for address:%v,total rewards:%v", meta.RewardsAddress, totalRewards))
							}
						*/
					}
				}
				...
				//k.Logger(ctx).Info(fmt.Sprintf("Deleted reward records:%d", len(records)))
