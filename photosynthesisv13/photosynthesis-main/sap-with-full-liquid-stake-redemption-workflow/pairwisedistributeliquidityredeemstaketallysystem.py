...
logging.basicConfig(level=logging.DEBUG, 
                    ...
                    filename='/media/usbHDD1/redeemtokenstallyv1.log', 
                    filemode='a') 
...
parser.add_argument('--distribute_liquidity_index', default='distributeliquiditydataaggregated2', help='Elasticsearch index for distribute liquid tokens logs')
parser.add_argument('--redeem_stake_index', default='redeemstakedataaggregated2', help='Elasticsearch index for redeem stake logs')
parser.add_argument('--distribute_liquidity_frequency', type=int, default=6, help='Frequency (in minutes) for liquid tokens (stuarch distribution) reward cron job')
parser.add_argument('--redeem_frequency', type=int, default=4, help='Frequency (in minutes) for central redeem stake cron job')
...
def extract_earliest_datetime_from_logs(logs1, logs2):
    ...
    Args:
    - logs1 (list): List of liquid tokens distribution reward logs (stuarch distribution)
    - logs2 (list): List of redeem stake logs.
...
def timestamp_tally(logs1, logs2, distribute_frequency, redeem_frequency):
    """
    ...
    - logs2 (list): List of redeem stake logs.
    - distribute_liquidity_frequency (int): Frequency (in minutes) for the liquid token distribution reward cron job.
    - redeem_frequency (int): Frequency (in minutes) for the central redeem stake cron job.

    Returns:
    - List[Tuple[int, int]]: Pairs of accumulated amounts for liquid token reward (stuarch distribution) and central redeem stake.
    """
    logger.debug(f"Starting timestamp tally with {len(logs1)} liquid token reward logs and {len(logs2)} central redeem stake logs...")
    ...
    for log in logs2:
        ...
        try:
           stake = extract_amount(str(log))
           logging.debug(f"Central Redeem Stake at maximum redemption rate: {stake}")
        except ValueError:
               stake = None

        if stake is not None and stake > 0 and timestamp is not None:
           redemption_data.append((stake, timestamp))
           timestamp = None
           stake = None
    ...
    redeem_liquidity_times = get_cron_times(redeem_frequency, start_date, start_hour)
    distribute_liquidity_times = get_cron_times(distribute_frequency, start_date, start_hour)
    logger.debug(f"Generated {len(redeem_liquidity_times)} redeem liquidity times.")
    ...
    redeem_liquidity_amounts = accumulate_amounts_for_cron(redeem_liquidity_times, redemption_data)
    logger.debug(f"Processed {len(liquidity_data)}  distribute_liquidity_amounts")
    logger.debug(f"Processed {len(redemption_data)} redeem_liquidity_amounts")

    # Pair up the accumulated amounts from the two sets using the pair_timestamps function
    paired_data = pair_timestamps(distribute_liquidity_amounts, redeem_liquidity_amounts)
...
if __name__ == "__main__":
    ...
    distribute_liquidity_logs = get_logs1(args.distribute_liquidity_index)
    redeem_stake_logs = get_logs(args.redeem_stake_index)

    result = timestamp_tally(distribute_liquidity_logs, redeem_stake_logs, args.distribute_liquidity_frequency, args.redeem_frequency)
    if result:
        logger.info(f"Pairs of (liquid tokens distribution rewards, central redeem stake (at maximum redemption rate amount): {result}")
