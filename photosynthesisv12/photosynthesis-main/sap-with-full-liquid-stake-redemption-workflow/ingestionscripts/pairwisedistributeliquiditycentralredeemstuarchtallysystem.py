...
logging.basicConfig(level=logging.DEBUG, 
                    ...
                    filename='/media/usbHDD1/redeemtokenstally.log', 
                    filemode='a') 
...
def timestamp_tally(logs1, logs2, distribute_frequency, redeem_frequency):
    ...
    """
    logger.debug(f"Starting timestamp tally with {len(logs1)} liquid token reward logs and {len(logs2)} central redeem stuarch (at maximum redemption rate) logs ...")
    ...
    amount = 0  
    # Process the redeem stuarch logs (at maximum redemption rate) (logs2)
    for log in logs2:
        try:
            timestamp = datetime.datetime.strptime(log, "%Y-%m-%dT%H:%M:%SZ")
...
if __name__ == "__main__":
    ...
    result = timestamp_tally(distribute_liquidity_logs, redeem_stuarch_logs, args.distribute_liquidity_frequency, args.redeem_frequency)
    if result:
        logger.info(f"Pairs of (liquid tokens distribution rewards (stuarch), central redeem cumulative stuarch (at maximum redemption rate)): {result}")
