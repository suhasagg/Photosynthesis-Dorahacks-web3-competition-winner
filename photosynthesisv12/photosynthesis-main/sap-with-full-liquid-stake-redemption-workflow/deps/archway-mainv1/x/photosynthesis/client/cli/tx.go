...
%s`, cmd.Long, setArchLiquidStakeIntervalHelp))
	...
	cmd.AddCommand(getCmdSetArchLiquidStakeInterval())
...
func GetCmdSetRedemptionInterval() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := photosynthesis.NewMsgSetRedemptionInterval(uint64(interval))
			if err := msg.ValidateBasic(); err != nil {
				return errors.Wrap(err, "invalid message")
			}
...
func setRedemptionRateQueryIntervalCmd(cdc *codec.Codec) *cobra.Command {
...
func setArchLiquidStakeIntervalCmd(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
