...
STRIDE_CHAIN_ID=stride # # DO NOT USE STRIDE MAINNET CHAIN ID! always use a new chain id by incrementing the suffix after each run
HOT_WALLET_1_MNEMONIC="tenant neck ask season exist hill churn rice convince shock modify evidence armor track army street stay light program harvest now settle feed wheat"
HOT_WALLET_2_MNEMONIC="tail forward era width glory magnet knock shiver cup broken turkey upgrade cigar story agent lake transfer misery sustain fragile parrot also air document"
HOT_WALLET_3_MNEMONIC="crime lumber parrot enforce chimney turtle wing iron scissors jealous indicate peace empty game host protect juice submit motor cause second picture nuclear area"
HOST_CHAIN_ID=localnet # chain id on the host
HOST_RPC=http://localhost  # RPC on the host
HOST_ACCOUNT_PREFIX=archway
HOST_DENOM=uatom
HOST_BINARY=build/archwayd
HOST_COIN_TYPE=118
...
$DOCKER_COMPOSE up -d photo1
$DOCKER_COMPOSE logs -f photo1 | sed -r -u "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2})?)?[mGK]//g" > $PHOTO_LOGS 2>&1 &

printf "Waiting for Photosynthesis to start..."
( tail -f -n0 $PHOTO_LOGS & ) | grep -q "executed block.*height="
echo "Done"
...
RELAYER_CMD="$SCRIPT_DIR/../../build/rly --home $STATE/relayer"
...
COMMANDS_FILE=${SCRIPT_DIR}/commandshost.sh
#cp ${SCRIPT_DIR}/templates/commandshost.sh $COMMANDS_FILE
...
bash ${SCRIPT_DIR}/commandshost.sh $STRIDE_CHAIN_ID
