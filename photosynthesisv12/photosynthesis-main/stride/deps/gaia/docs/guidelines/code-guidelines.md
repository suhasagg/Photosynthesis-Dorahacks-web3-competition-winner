...
- Does the caller need to match the error to handle it? If yes, we must support
...
For an error with a dynamic string, use [`fmt.Errorf`][fmt.Errorf] if the caller does not
