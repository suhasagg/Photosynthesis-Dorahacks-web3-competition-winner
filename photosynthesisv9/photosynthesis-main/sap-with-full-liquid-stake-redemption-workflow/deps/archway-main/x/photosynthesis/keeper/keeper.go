...
func (k PhotosynthesisKeeper) DistributeRedeemedTokens(ctx sdk.Context, epoch int64) (float64, error) {
	k.Logger(ctx).Info(fmt.Sprintf("Start Distribute Redeemed Amount:%v", epoch))
	...
	k.Logger(ctx).Info(fmt.Sprintf("Distribute redemed amount:%v", redeemedAmount))
	contractmeta := k.rewardKeeper.GetState().ContractMetadataState(ctx)
	cumulativeStakes := make(map[string]float64)
	totalStake := 0.0
	// Initialize map

	filePath := "/home/photo/logs/liquiditydistributionforDapps"
	var lastMatchingOffset int64 = -1 // track the offset of last matching line
	var processRecords bool = true    // flag to indicate when to process records
	var count int = 0                 // counter for matching lines

	// Read file content to count occurrences in advance
	...
	pattern := regexp.MustCompile("Number of successfully claimed records")
	matches := pattern.FindAllString(string(content), -1)
	matchLimit := len(matches) // stop processing after this number of matches
	k.Logger(ctx).Info(fmt.Sprintf("Number of successfully claimed epoch unbonding records:%d", matchLimit))
	pattern1 := regexp.MustCompile("Number of successfully redeemed records")
	matches1 := pattern1.FindAllString(string(content), -1)
	matchLimit1 := len(matches1) // stop processing after this number of matches
	k.Logger(ctx).Info(fmt.Sprintf("Number of successfully redeemed records:%d", matchLimit1))
	...
	for scanner.Scan() {
		...
		// If line matches the pattern, update the offset and count
		if pattern1.MatchString(line) {
			lastMatchingOffset = pos // Update the offset of last matching line
			count++
			// Stop processing records if count reaches limit
			if count == matchLimit {
		...
		if processRecords {
			// Split the line into parts and process
			parts := strings.Split(line, ",")
			if len(parts) == 4 {
				amount, err1 := strconv.ParseFloat(parts[3], 64)
				if err1 != nil {
					fmt.Println("Error converting to float", err1)
					continue
				}

				if _, exists := dappStakes[parts[1]]; exists {
					dappStakes[parts[1]] += amount
				} else {
					dappStakes[parts[1]] = amount
				}
			}
	...
	if lastMatchingOffset != -1 {
		// Reopen the file and truncate it at the last matching offset
		file, err = os.OpenFile(filePath, os.O_RDWR, 0644)
		if err != nil {
			fmt.Println("Error opening file", err)
		}

		err = file.Truncate(lastMatchingOffset)
		if err != nil {
	...
	for contractAddr, contractStake := range cumulativeStakes {
		...
		k.Logger(ctx).Info(fmt.Sprintf("Stake ratio:%v", stakeratio))
		// Calculate the amount of liquidity tokens to distribute for the current contract
		...
		k.Logger(ctx).Info(fmt.Sprintf("Contract Address:%s,Redeemed Token Amount:%v", contractAddress, redeemedTokensAmount))
		if err != nil {
...
func (k PhotosynthesisKeeper) GetCumulativeRewardAmount(ctx sdk.Context, epoch int64, rewardaddr sdk.AccAddress) int64 {
	...
	//	}
	//}
	k.Logger(ctx).Info(fmt.Sprintf("Finish Get Cumulative Reward Amount epoch:%v,rewardaddr:%v", epoch, rewardaddr))
