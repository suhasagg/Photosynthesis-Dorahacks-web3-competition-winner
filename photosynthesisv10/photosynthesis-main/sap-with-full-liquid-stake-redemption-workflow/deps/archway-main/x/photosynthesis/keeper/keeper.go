...
func (k PhotosynthesisKeeper) GetCumulativeRewardAmount(ctx sdk.Context, epoch int64, rewardaddr sdk.AccAddress) map[string]int {
	...
	for scanner.Scan() {
		...
		if err != nil {
			k.Logger(ctx).Error(fmt.Sprintf("Error in reading rewards amount:%v", err))
			return nil
		}
				...
				if meta.MinimumRewardAmount > 0 {
					...
					if info.CurrentEpoch != 0 && info.CurrentEpoch%int64(meta.LiquidStakeInterval) == 0 {
						...
						if meta.RewardsAddress != "" {
							...
							rewardMap = k.GetCumulativeRewardAmount(ctx, info.CurrentEpoch, sdk.AccAddress(meta.RewardsAddress))
							_, exists := rewardMap[meta.RewardsAddress]
							if !exists {
								k.Logger(ctx).Info(fmt.Sprintf("RewardsAddress not found in rewardMap"))
								continue
							}
