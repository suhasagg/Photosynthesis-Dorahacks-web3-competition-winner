	...
	*/
	/*
		...
		cmd := []string{
			...
			"bank",
			"balances",
			"archway17zyrf6u6fltey79rz4h02c6ydmcpctyn3nf9vf",
		}
	...
	*/
	...
	for i := len(lines) - 1; i >= 0; i-- {
		if denomMatch := denomRegex.FindString(lines[i]); denomMatch != "" {
			k.Logger(ctx).Info(fmt.Sprintf("Contract Address: %s, Liquid Tokens Obtained Amount: %v", "archway17zyrf6u6fltey79rz4h02c6ydmcpctyn3nf9vf", amount))
	...
	for contractAddr, contractStake := range cumulativeStakes {
		...
		//err = k.bankkeeper.SendCoins(ctx, sdk.AccAddress("archway17zyrf6u6fltey79rz4h02c6ydmcpctyn3nf9vf"), sdk.AccAddress(contractAddr), sdk.NewCoins(sdk.NewCoin("ibc/15CE03505E1F9891F448F53C9A06FD6C6AF9E5BE7CBB0A4B45F7BE5C9CBFC145", sdk.NewInt(int64(liquidityTokensAmount)))))
		data := "archway17zyrf6u6fltey79rz4h02c6ydmcpctyn3nf9vf" + "," + contractAddr + "," + "ibc/15CE03505E1F9891F448F53C9A06FD6C6AF9E5BE7CBB0A4B45F7BE5C9CBFC145" + "," + strconv.FormatFloat(liquidityTokensAmount, 'f', 2, 64)
...
func (k PhotosynthesisKeeper) GetCumulativeLiquidityAmount(ctx sdk.Context, epoch int64, centralliquidityAddr sdk.AccAddress) (sdk.Int, error) {
	...
	contractmeta.IterateContractMetadata(func(meta *rewardstypes.ContractMetadata) (stop bool) {
		...
		text := meta.LiquidityTokenAddress + "," + centralliquidityAddr.String()
		data := []byte(text + "\n")
		...
		/*
			for _, balance := range data {
				if balance.Denom == "ibc/15CE03505E1F9891F448F53C9A06FD6C6AF9E5BE7CBB0A4B45F7BE5C9CBFC145" {
					...
					k.Logger(ctx).Info(fmt.Sprintf("Contract Address: %s, Liquid Token ibc/15CE03505E1F9891F448F53C9A06FD6C6AF9E5BE7CBB0A4B45F7BE5C9CBFC145 Amount: %v", "archway17zyrf6u6fltey79rz4h02c6ydmcpctyn3nf9vf", ibcliquidityAmount))
				...
				if balance.Denom == "stuarch" {
					...
					k.Logger(ctx).Info(fmt.Sprintf("Contract Address: %s, Liquid Token stuarch Amount: %v", "archway17zyrf6u6fltey79rz4h02c6ydmcpctyn3nf9vf", stuarchliquidityAmount))
