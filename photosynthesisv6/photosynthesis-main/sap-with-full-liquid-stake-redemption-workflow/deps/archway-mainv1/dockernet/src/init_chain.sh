...
for (( i=1; i <= $NUM_NODES; i++ )); do
    ...
    if [[ $chain_id == "localnet" ]]; then
        val_mnemonic="material isolate early jungle measure pull cheese impact spell gossip where field nature matrix note stuff satoshi exclude dawn warfare double ability retire wing format shell"
    else
        val_mnemonic="${VAL_MNEMONICS[((i-1))]}"
    fi

    echo "val_mnemonic: $val_mnemonic"
