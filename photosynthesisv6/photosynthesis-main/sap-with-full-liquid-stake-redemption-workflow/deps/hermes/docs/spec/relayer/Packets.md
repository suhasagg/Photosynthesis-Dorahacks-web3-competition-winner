...
This document specifies datagram creation logic for packets. It is used by the
relayer.
...
Successful handling of *SendPacketEvent* leads to *PacketRecv* datagram
creation.

// NOTE: Stateful relayer might keep packet that are not acked in the state so
the following logic // can be a bit simpler.
...
Successful handling of *WriteAcknowledgementEvent* leads to
*PacketAcknowledgement* datagram creation.
