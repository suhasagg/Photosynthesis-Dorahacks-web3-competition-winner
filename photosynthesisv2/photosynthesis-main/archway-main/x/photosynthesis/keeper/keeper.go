...
func (k PhotosynthesisKeeper) GetTotalLiquidStake(ctx sdk.Context, epoch int64) (sdk.Int, error) {
	...
	for _, contract := range contracts {
		...
		depositRecords, err := k.GetContractLiquidStakeDepositsTillEpoch(ctx, sdk.AccAddress(contract.Address), epoch)
		if err != nil {
...
func (k PhotosynthesisKeeper) RedeemLiquidTokens(ctx sdk.Context, cumLiquidityAmount *types.Coin) (int64, error) {
	...
	for _, contract := range contracts {
		...
		meta := k.rewardKeeper.GetContractMetadata(ctx, sdk.AccAddress(contract.Address))
		...
		err := k.bankKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, sdk.AccAddress(contract.Address), coin)
		if err != nil {
			return 0, err
		}
...
func (k PhotosynthesisKeeper) GetTotalStake(ctx sdk.Context) (*types.Coin, error) {
	...
	contracts, _ := k.ListContracts(ctx)
	for _, contract := range contracts {
		lstake, err := k.GetStake(ctx, sdk.AccAddress(contract.Address))
		if err != nil {
...
func (k PhotosynthesisKeeper) GetCumulativeRewardAmount(ctx sdk.Context, contractAddress string) sdk.Coins {
	records, _, _ := k.rewardKeeper.GetRewardsRecords(ctx, sdk.AccAddress(contractAddress), nil)
			...
			case epochstypes.LIQUID_STAKING_DApp_Rewards_EPOCH:
				...
				if meta.MinimumRewardAmount > 0 {
					if info.CurrentEpoch%int64(meta.LiquidStakeInterval) == 0 {
						...
						rewardAmount := k.GetCumulativeRewardAmount(ctx, contract.Address)
						if rewardAmount.AmountOf(rewardAmount.GetDenomByIndex(0)).Int64() >= int64(meta.MinimumRewardAmount) {
							record := k.CreateContractLiquidStakeDepositRecordsForEpoch(ctx, sdk.AccAddress(meta.ContractAddress), info.CurrentEpoch)
