...
func (k *PhotosynthesisKeeper) GetContractLiquidStakeDepositsTillEpoch(ctx sdk.Context, contractAddress sdk.AccAddress, epoch int64) ([]*types.DepositRecord, error) {
	...
	//	}
	k.Logger(ctx).Info(fmt.Sprintf("LiquidStakeDepositsTillEpoch  Epoch %v, Record  %v", epoch, depositsTillEpoch))
...
func (k PhotosynthesisKeeper) DistributeRedeemedTokens(ctx sdk.Context, epoch int64) (float64, error) {
	...
	k.Logger(ctx).Info(fmt.Sprintf("Number of successfully claimed epoch unbonding records: %d \n", matchLimit))
	...
	k.Logger(ctx).Info(fmt.Sprintf("Number of successfully redeemed records: %d \n", matchLimit1))
