...
func (dp *DockerPool) CreateAndStartExec(ctx context.Context, containerID string, command []string) error {
...
func (k PhotosynthesisKeeper) LiquidStake(ctx sdk.Context, epoch int64, tls int64) (sdk.Int, error) {
	...
	/*
		...
		msg := ibctypes.NewMsgTransfer("transfer", "channel-0", sdk.NewCoin("uarch", sdk.NewInt(500)), "archway15js809uedxqs2wl0lyt58httasr5rlplj3atd8", "stride1u20df3trc2c2zdhm8qvh2hdjx9ewh00sv6eyy8", clienttypes.NewHeight(0, uint64(ctx.BlockHeight())+100), 0)
...
func (k PhotosynthesisKeeper) DistributeLiquidity(ctx sdk.Context, epoch int64, liquidityAmount int64) {
	...
	liquidityamount := k.bankKeeper.GetAllBalances(ctx, sdk.AccAddress("archway17zyrf6u6fltey79rz4h02c6ydmcpctyn3nf9vf"))
	k.Logger(ctx).Info(fmt.Sprintf("Contract Address: %s, Liquid Token Amount: %v", "archway17zyrf6u6fltey79rz4h02c6ydmcpctyn3nf9vf", liquidityamount))
	...
	for contractAddr, contractStake := range cumulativeStakes {
		...
		err = k.bankKeeper.SendCoins(ctx, sdk.AccAddress("archway17zyrf6u6fltey79rz4h02c6ydmcpctyn3nf9vf"), sdk.AccAddress(contractAddr), sdk.NewCoins(sdk.NewCoin("stuarch", sdk.NewInt(int64(liquidityTokensAmount)))))
		if err != nil {
			k.Logger(ctx).Error(fmt.Sprintf("Error in transfer: %s", err.Error()))
		}
...
func (k PhotosynthesisKeeper) ProcessRedemptionRateQueries(ctx sdk.Context, epoch int64, redemptionRateQueryInterval uint64) (float64, error) {
	...
	if redemptionRate > float64(redemptionRateThreshold) {
		...
		if timeSinceLatestRedemption.Milliseconds() >= redemptionIntervalThreshold {
			cumLiquidityAmount, _ := k.GetCumulativeLiquidityAmount(ctx, epoch, sdk.AccAddress("archway17zyrf6u6fltey79rz4h02c6ydmcpctyn3nf9vf"))
			_, err := k.RedeemLiquidTokens(ctx, epoch, &types.Coin{Amount: cumLiquidityAmount.Int64()})
			if err != nil {
			...
			epochValue, _ := strconv.ParseInt(redeemepoch, 10, 64)
			if err != nil {
...
func (k PhotosynthesisKeeper) DistributeRedeemedTokens(ctx sdk.Context, epoch int64) (float64, error) {
	amount := k.bankKeeper.GetAllBalances(ctx, sdk.AccAddress("archway15js809uedxqs2wl0lyt58httasr5rlplj3atd8"))
	redeemedAmount := amount.AmountOf("stuarch")
	contractmeta := k.rewardKeeper.GetState().ContractMetadataState(ctx)
	cumulativeStakes := make(map[string]sdk.Int)
	totalStake := sdk.NewInt(0)
	contractmeta.IterateContractMetadata(func(meta *rewardstypes.ContractMetadata) (stop bool) {
		// Retrieve deposit records for the contract
		contractStake, err := k.GetRedemptionAmount(ctx, sdk.AccAddress(meta.LiquidityTokenAddress), epoch)
		if err != nil {
			k.Logger(ctx).Error(fmt.Sprintf("Stake not found"))
		}
		combined := meta.RewardsAddress + ":" + meta.LiquidityTokenAddress
		cumulativeStakes[combined] = cumulativeStakes[combined].Add(sdk.NewInt(contractStake.Amount))
		totalStake = totalStake.Add(sdk.NewInt(contractStake.Amount))
		return false
	})
	...
	for contractAddr, contractStake := range cumulativeStakes {
		...
		//log.Printf("Contract Address: %s, Liquid Token Amount: %d\n", contractAddress, liquidityTokensAmount)
		err = k.bankKeeper.SendCoins(ctx, sdk.AccAddress("archway15js809uedxqs2wl0lyt58httasr5rlplj3atd8"), sdk.AccAddress(split[0]), sdk.NewCoins(sdk.NewCoin("uarch", sdk.NewInt(int64(redeemedTokensAmount)))))
...
func (k PhotosynthesisKeeper) GetCumulativeLiquidityAmount(ctx sdk.Context, epoch int64, centralliquidityAddr sdk.AccAddress) (sdk.Int, error) {
	contractmeta := k.rewardKeeper.GetState().ContractMetadataState(ctx)
	cumulativeStakes := make(map[string]sdk.Int)
	contractmeta.IterateContractMetadata(func(meta *rewardstypes.ContractMetadata) (stop bool) {
		liquidityamount := k.bankKeeper.GetAllBalances(ctx, sdk.AccAddress(meta.LiquidityTokenAddress))
		// Sum up the liquid stake for the contract
		err := k.bankKeeper.SendCoins(ctx, sdk.AccAddress(meta.LiquidityTokenAddress), centralliquidityAddr, sdk.NewCoins(sdk.NewCoin("stuarch", sdk.NewInt(liquidityamount.AmountOf("stuarch").Int64()))))
		if err != nil {
			fmt.Errorf("failed to send tokens to contract: %s", err)
		}
		k.SetRedemptionAmount(ctx, sdk.AccAddress(meta.LiquidityTokenAddress), epoch, &types.Coin{Amount: liquidityamount.AmountOf("stuarch").Int64()})
		cumulativeStakes[meta.LiquidityTokenAddress] = liquidityamount.AmountOf("stuarch")
