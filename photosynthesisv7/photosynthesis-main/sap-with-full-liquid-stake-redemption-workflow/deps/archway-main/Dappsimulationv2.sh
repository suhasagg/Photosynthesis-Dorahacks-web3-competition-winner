...
OUTPUT1=$(build/archwayd --home dockernet/state/photo1 tx wasm store deps/archway-main/contrib/localnet/testdata/cw20_base.wasm --from pval1 --node http://$output:26657 --keyring-backend test --fees 500000uarch --gas 2000000 --chain-id localnet -y)
...
# Instantiate
OUTPUT=$(build/archwayd --home dockernet/state/photo1 --node http://$output:26657 tx wasm instantiate 1 "{\"reward_address\": \"$REWARD_ADDRESS\", \"instantiation_request\": \"e30=\", \"gas_rebate_to_user\": false, \"collect_premium\": false, \"premium_percentage_charged\": 0, \"name\": \"aaaaa\", \"symbol\": \"TOK\", \"decimals\": 6, \"initial_balances\": [], \"mint\": {\"minter\": \"$MINTER_ADDRESS\"}}" --label test --from pval1  --keyring-backend=test --chain-id localnet --fees 10000uarch --no-admin -y)
...
OUTPUT=$(build/archwayd --home dockernet/state/photo1 q rewards rewards-records "$reward_address")
